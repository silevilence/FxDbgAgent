...
- [ ] **阶段4-1 受限表达式求值**——安全评估通过后实现只读、限时、可取消的表达式求值
    - [ ] 前置门禁：完成安全评估并输出受限求值边界（允许/禁止清单、不改变业务状态的保证、资源上限、失败与超时模式），作为 ADR 记录进 `docs/architecture.md` 并与用户确认；评估通过前不实现、不暴露公开入口
    - [ ] 求值范围：仅限 stopped 状态下基于指定线程/帧上下文求值；支持字面量、字段/数组/索引访问、算术/比较/逻辑运算与受控字符串操作；仅允许白名单内无副作用的只读方法（如 mscorlib 的 String/Array/Math 静态成员），禁止任意用户方法调用、属性 Getter、Setter/变量修改、反射与动态调用
    - [ ] 只读保证：求值不得改变业务状态——求值前后目标字段/静态/对象状态一致，不调用任何可写路径；结果显示沿用既有深度、成员数、字符串长度与分页上限，不调用目标自定义格式化代码
    - [ ] 执行纪律：求值经既有单调度线程发起、等待 Eval 完成回调，回调仍只入队；有严格超时与取消，超时/取消后会话状态一致且可继续；禁止在回调线程求值或阻塞调度线程；恢复运行后旧帧/引用失效语义不变
    - [ ] 入口复用：CLI、MCP、DAP 复用同一 Host/Engine 求值路径；MCP 以新增工具或可选参数扩展并保持既有 13 工具调用兼容，同步工具契约、CLI 命令、DAP evaluate 能力声明与 Agent 技能文档
    - 验收：真实 x86/x64 × Debug/Release 样例在停止态完成受限表达式求值，结果与变量模型一致；用带副作用计数与状态字段的样例证明求值前后业务状态不变、白名单外路径调用计数为零；对死循环/极慢表达式验证超时与取消可靠且会话可继续、无重复 Continue 或死锁；语法错误、上下文不可用、引用失效、超时、取消错误分类明确且不泄漏堆栈；既有 13 工具与 CLI/DAP 全量回归通过。

- [ ] **阶段4-2 first-chance exception 过滤**——按异常类型配置 first-chance 停止条件
    - [ ] 在既有会话级 first-chance 开关（CLI `exceptions --first-chance`、Engine `exceptions.configure`，默认关闭）之上增加按类型过滤：配置一个或多个匹配规则（精确类型名、命名空间前缀、基类/派生匹配，语义文档化），仅匹配类型在 first-chance 停止，未匹配类型不停止
    - [ ] 过滤规则可动态更新（停止态或按既有运行中安全规则），清空/关闭后恢复默认仅未处理异常；异常报告字段（类型、消息、线程、抛出位置、调用栈）沿用阶段 1-8，消息仍只读 `_message` 字段，不调用自定义格式化代码
    - [ ] 高频约束：first-chance 回调只入队，类型匹配判断不得在回调线程执行耗时工作或触发求值；未匹配异常不产生停止、不完成运行操作、不污染状态观察；过滤开启不得引入无界队列积压或拖慢停止响应
    - [ ] 入口复用：CLI 扩展 `exceptions` 命令、Engine 协议扩展；MCP 以可选参数或新工具暴露会话级过滤配置（保持既有调用兼容），DAP 按声明能力扩展 `setExceptionBreakpoints`；同步文档与 Agent 技能
...
- [ ] **阶段4-3 条件断点**——命中次数条件独立实现，表达式条件依赖 4-1
    - [ ] 断点增加可选条件：命中次数（阈值语义文档化）与/或受限表达式（AND 组合）；无条件的既有断点行为完全不变，断点状态机仍仅 pending/verified/moved/unresolved，沿用模块加载或 PDB 就绪后自动重绑定
    - [ ] 命中次数：仅在实际命中时计数，达到阈值才作为用户停止返回；断点删除后计数重置，模块重载/重绑定保持同一断点计数；计数与条件状态在断点查询中可见
    - [ ] 表达式条件：仅允许 4-1 定义的受限表达式（不执行用户代码、不改状态、限时），结果必须为布尔可判定；条件不满足时内部自动继续——不得向用户暴露该次内部停止、不得完成 waitForStop 运行操作、不得产生停止事件；条件求值失败/超时按明确策略处理并留诊断，不得静默或造成命中-求值-再命中死循环
    - [ ] 入口复用：`debug_set_breakpoint` 以可选参数扩展条件字段，CLI `break` 与 DAP conditionalBreakpoints 能力同步扩展并声明；同步 `docs/mcp-tools.md`、`docs/cli.md`、`docs/dap.md` 与 Agent 技能
