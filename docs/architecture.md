...
## ADR-005：受限表达式安全扩展（2026-09-10，已接受）

### 门禁结论

本ADR只扩展ADR-004的允许清单，不推翻其任何不变量与既有决策：继续不创建ICorDebugEval、不执行目标方法/Getter、不新增Continue、不写目标内存。用户于2026-09-10在设计评审中批准四项方向（语法增强、内置函数增强、IL证明的属性读取、诊断自愈）及“IL证明”机制，并于同日复核确认本ADR文本。实施按ROADMAP「计划中」条目依序推进，未通过验收前不勾选。原需求文档与“MVP禁止函数求值”决策不变，扩展仅限只读观察的表达能力。

枚举名解码不纳入本轮：ClrDebug 0.4.2（固定依赖）实测未包装 `GetConstantProps`/`TryGetConstantProps`，枚举常量表不可达；恢复评估需自研COM interop片段或原始元数据解析，另行决策，不阻塞本轮实施。

### 决策内容

- **语法**：三元 `?:`；位运算 `& | ^ ~ << >>`（按C#优先级）；`0x` 十六进制字面量；基础数值显式转换 `(T)x`（C# unchecked 语义并文档化）；裸名解析回退——根（参数/局部/this）未命中时按 `this` 成员重试（字段优先，其次属性证明）。
- **内置函数**（解释器固有操作，不调用目标mscorlib）：`String.Substring/IndexOf/Contains/StartsWith/EndsWith/Trim/CompareOrdinal/IsNullOrWhiteSpace`；`Math.Round/Floor/Ceiling/Truncate/Sqrt/Sign/Clamp`；`Array.IndexOf`。全部只作用于已读值。
- **属性读取（IL证明的trivial getter）**：接收者按其运行时类型（ExactType）沿类型层级解析零参数 getter；读取其IL字节（单方法≤16字节），仅接受精确模式：实例 `ldarg.0; ldfld <fieldToken>; ret`、静态 `ldsfld <fieldToken>; ret`、常量 `ldc.*/ldnull; ret`；IL不得含分支、调用、异常子句或多余指令。fieldToken经元数据解析（FieldDef/MemberRef）到接收者层级中的字段槽（优先 token+模块匹配，回退名称匹配，歧义即拒绝），随后复用既有字段读取路径。字段与属性同名时字段优先；按ExactType解析使虚拟/重写getter的语义与运行时分派一致，无需额外限制；显式接口实现（`IFace.get_X`）与带参数索引器拒绝。
- **诊断自愈**：成员解析失败时返回候选成员名（字段名+属性名，经前缀/大小写/编辑距离排序，上限8条，只含元数据名称、绝不含运行值）；属性被拒时说明“getter 无法证明只读，不会被调用”。七个错误码冻结不变，仅富化消息——本项是对“错误不回显表达式”脱敏边界的一次微调：允许回显**类型成员名（元数据）**，仍禁止表达式文本、变量值与内部堆栈。

### 资源与预算

- 既有上限全部维持：输入4096代码单元、1024语法节点、32层、10000步骤、1024次值读取、单字符串32768/累计中间串65536、期限1～1000ms（含排队）。
- 新增：元数据探针≤256次/表达式；IL字节单方法≤16、累计≤256/表达式；类型级属性解析缓存绑定停止代次，随stop失效，构建计入探针预算；候选诊断计算计入步骤预算；超限按 `expression_limit_exceeded` 拒绝。

### 安全论证与拒绝用例

- IL证明链路只读元数据与既有内存值：不创建Eval、不Continue、不调用目标方法——getter从不被执行，只读其IL字节与其声明的字段；ExactType保证“读到的字段就是运行时分派会执行的实现”，虚拟属性不引入语义偏差。
- 必须拒绝并留稳定诊断的用例：IL含分支/调用/多字段加载/超长；getter带参数；抽象或无IL；显式接口实现；token不唯一；值类型接收者读取失败。拒绝不改变会话状态，可改写表达式重试。
- 验收证据：目标侧副作用计数在接受与拒绝路径前后不变；Eval/目标方法/类型初始化器调用计数为零；Continue配对计数不变；4-3断点条件自动继承本扩展语法并专项回归。

### 后继任务与验收

- 实施顺序：语法与内置函数（Core）→ IL证明属性读取（Interop）→ 诊断自愈 → 全量验证与独立审核。每项依序实施，真实x86/x64 × Debug/Release验收，快速审核仅修阻塞，通过后原地勾选并本地提交。
- 全量回归包含真实Service/IIS、VS Code及阶段4/3/2/1/0；变更生产行覆盖率目标≥90%；`docs/mcp-tools.md` 与 `skills/fxdbg-agent/references/` 逐字节同步（`eng/verify-skill-install.ps1` 校验）；既有15个工具契约只增不改。
