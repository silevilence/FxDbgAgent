...
public sealed class RealExpressionCoverage
{
    [Theory]
    ...
    public void Existing_real_matrix_passes_under_coverage(string entry, string suite)
    {
        ...
        string logs = Path.Combine(root, "artifacts/expression-extension/coverage", configuration); Directory.CreateDirectory(logs);
