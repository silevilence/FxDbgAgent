...
$diff=git -C $RepoRoot -c "safe.directory=$safeDirectory" diff $manifest.baseline $manifest.reviewedCommit --unified=0 -- src
if($LASTEXITCODE -ne 0) { throw 'Cannot read the fixed review diff.' }
foreach($text in $diff) {
    if($text -match '^\+\+\+ b/(.+)$') { $current=$Matches[1];$changed[$current]=[Collections.Generic.HashSet[int]]::new() }
...
@{coverageFiles=@($coverageFiles.FullName);covered=$covered;total=$total;rate=$covered/$total;files=$rows;incompleteChangedMethods=$incompleteMethods;overallProduction=@{covered=$allCovered;total=$allTotal;rate=$allCovered/$allTotal}} | ConvertTo-Json -Depth 7 | Set-Content (Join-Path $repoRoot 'artifacts/stage3-4-validation/coverage-changes.json')
