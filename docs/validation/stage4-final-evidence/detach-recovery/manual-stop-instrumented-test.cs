...
internal static partial class Program
{
    ...
    {
        foreach (string scenario in new[] { "UnloadModule", "ExitAppDomain", "DelayedUnloadModule" })
        {
            string callbackKind = scenario == "DelayedUnloadModule" ? "UnloadModule" : scenario;
            ...
            try
            {
                ...
                if (scenario == "DelayedUnloadModule")
                {
                    // Delay only delivery of this real envelope: Stop can race with its arrival.
                    // The producer performs no native/debuggee operations.
                    object held = queue.GetType().GetMethod("Take", Type.EmptyTypes)!.Invoke(queue, null)!;
                    Thread.Sleep(120); // Expire any quiet window from earlier, unrelated module callbacks.
                    MethodInfo add = queue.GetType().GetMethod("Add", new[] { held.GetType() })!;
                    producer = new Thread(() =>
                    {
                        try { Thread.Sleep(20); add.Invoke(queue, new[] { held }); }
                        catch (Exception error) { producerFailure = error; }
                    });
                    producer.Start();
                }
                try { session.Detach(); }
                ...
                {
                    if (producer is not null) Require(producer.Join(5000), "Delayed callback producer must exit.");
                }
                if (producerFailure is not null) throw producerFailure;
                ...
                Console.WriteLine("PASS: detach drains queued " + scenario + " and invalidates native bindings (" + (IntPtr.Size * 8) + " bit).");
            }
