...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $repoRoot "artifacts/mcp/$Configuration" }
...
dotnet publish (Join-Path $repoRoot 'src/FxDbg.McpHost/FxDbg.McpHost.csproj') -c $Configuration --no-restore --no-build -o $OutputDirectory
...
Write-Host "MCP bundle: $OutputDirectory"
