...
function Get-ValidationInputs {
    foreach ($directory in @('src','tests','eng','extensions','skills')) {
        Get-ChildItem -LiteralPath (Join-Path $repoRoot $directory) -Recurse -File |
            Where-Object { $_.FullName -notmatch '[\\/](bin|obj|node_modules)[\\/]' -and $_.Extension -in @('.cs','.csproj','.props','.targets','.ps1','.mjs','.js','.json','.md') } |
...
function Invoke-Validation([string]$Name, [string[]]$Arguments, [int]$Seconds) {
    ...
    $code = [FxDbg.Validation.ValidationProcess]::Run($shell, $Arguments, $repoRoot, $log, $Seconds)
...
try {
    ...
    @{ passed = $passed; configuration = $(if ($Configuration) { $Configuration } else { 'Debug+Release' }); skipped = @('3-4');
...
Write-Host 'Stage 3-1/3-2/3-3/3-5 and complete stage 2/1 regressions passed. Stage 3-4 remains skipped.'
