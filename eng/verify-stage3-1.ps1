...
foreach ($current in $configurations) {
    $code = [FxDbg.Validation.ValidationProcess]::Run($shell,
        ...
        $repoRoot, (Join-Path $report "paging-build-$current.log"), 600)
    if ($code -ne 0) { throw "Paging build failed: $current ($code)" }
    ...
    $code = [FxDbg.Validation.ValidationProcess]::Run('dotnet',
        ...
        $repoRoot, (Join-Path $report "paging-unit-$current.log"), 120)
    if ($code -ne 0) { throw "Paging unit tests failed: $current ($code)" }
    $code = [FxDbg.Validation.ValidationProcess]::Run('dotnet',
        @((Join-Path $repoRoot "tests/FxDbg.McpIntegrationTests/bin/$current/net10.0-windows/FxDbg.McpIntegrationTests.dll"),
          (Join-Path $repoRoot "artifacts/mcp/$current"),'paging',$repoRoot,$current),
        $repoRoot, (Join-Path $report "paging-mcp-$current.log"), 300)
    Get-Content (Join-Path $report "paging-mcp-$current.log") | Write-Host
    if ($code -ne 0) { throw "Paging MCP tests failed: $current ($code)" }
}
