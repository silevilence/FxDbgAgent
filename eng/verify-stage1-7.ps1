...
try {
    ...
    foreach ($architecture in @('x86', 'x64')) {
        ...
        & $runner $repoRoot $Configuration variables
        if ($LASTEXITCODE -ne 0) { throw "Real $architecture variable inspection failed." }
    }
    Write-Host 'Stage 1-7 verification passed.'
}
