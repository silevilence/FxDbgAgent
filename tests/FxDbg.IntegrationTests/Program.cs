...
internal static partial class Program
{
    private static int Main(string[] args)
    {
        try
        {
            if (args.Length > 2 && args[2] == "queued-unload-detach") { RunQueuedUnloadDetach(Path.GetFullPath(args[0]), args[1]); return 0; }
            if (args.Length > 2 && args[2] == "conditional-breakpoints") { RunConditionalBreakpoints(Path.GetFullPath(args[0]),args[1]); return 0; }
            ...
            RunQueuedUnloadDetach(Path.GetFullPath(args[0]), args.Length > 1 ? args[1] : "Debug");
