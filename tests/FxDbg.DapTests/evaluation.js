...
async function main() {
  ...
  for (const architecture of ['x86','x64']) {
    ...
    try {
      ...
      for(const [expression,expected] of [['number + 2','44'],['(number & 0x1) == 0 ? Math.Clamp(number,0,10) : 0','10'],['(byte)257','1'],['String.Substring(message,2,3)','cde'],['fallbackNumber','17'],['Array.IndexOf(Values,20)','1'],['matrix[1,1]','40'],['shifted[-1,7]','99'],['Properties.Promoted','5'],['List.Count','3'],['Properties.Pure','23'],['Virtual.Value','23'],['Properties.Static','37'],['Struct.Pure','31'],['Properties.Flag','true'],['userCodeCalls','0']]) {
      ...
      for (const expression of ['OddVirtual.Value','ImplicitVirtual.Value','Dictionary.Count','Properties.Computed','Properties.SideEffect','ComputedVirtual.Value','Properties.Item','Properties.Explicit','Properties.Cold','Generic.OtherInstantiation']) await assert.rejects(client.request('evaluate',{frameId,expression}),/expression_name_not_found/);
    ...
    try {
      ...
      for (const [expression,expected] of [['(number & 0x1) == 0 ? Math.Clamp(number,0,10) : 0','10'],['fallbackNumber','17'],['String.Substring(message,2,3)','cde'],['Array.IndexOf(Values,20)','1'],['Properties.Promoted','5'],['List.Count','3'],['Properties.Pure','23'],['Virtual.Value','23'],['Properties.Static','37'],['Struct.Pure','31'],['Properties.Flag','true'],['userCodeCalls','0']]) assert.equal(call('evaluate',['--frame',frame,'--expression',expression]).result.displayValue,expected);
